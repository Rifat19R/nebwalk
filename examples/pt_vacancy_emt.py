"""
Pt vacancy migration in FCC Pt — EMT calculator.

Reference barrier: ~1.49 eV (DFT-PBE).

Usage
-----
    python examples/pt_vacancy_emt.py
"""
...
pt_bulk = bulk("Pt", "fcc", a=3.924, cubic=True)
supercell = pt_bulk.repeat(2)        # 2x2x2 → 32 atoms
...
print(f"NN distance    : {nn_dist:.4f} Å  (expected ~2.775 Å for FCC Pt)")
...
neb.plot("pt_vacancy_emt_profile.png",
         title="Pt vacancy migration in FCC Pt [EMT, CI-NEB]")
neb.save_csv("pt_vacancy_emt_profile.csv")
neb.save_trajectory("pt_vacancy_emt.traj")
