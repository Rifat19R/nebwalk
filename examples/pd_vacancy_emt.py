"""
Pd vacancy migration in FCC Pd — EMT calculator.

Reference barrier: ~0.91 eV (DFT-PBE).

Usage
-----
    python examples/pd_vacancy_emt.py
"""
...
pd_bulk = bulk("Pd", "fcc", a=3.890, cubic=True)
supercell = pd_bulk.repeat(2)        # 2x2x2 → 32 atoms
...
print(f"NN distance    : {nn_dist:.4f} Å  (expected ~2.751 Å for FCC Pd)")
...
neb.plot("pd_vacancy_emt_profile.png",
         title="Pd vacancy migration in FCC Pd [EMT, CI-NEB]")
neb.save_csv("pd_vacancy_emt_profile.csv")
neb.save_trajectory("pd_vacancy_emt.traj")
