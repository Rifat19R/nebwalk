"""
Ni vacancy migration in FCC Ni — EMT calculator.

Reference barrier: ~1.04 eV (DFT-PBE).

Usage
-----
    python examples/ni_vacancy_emt.py
"""
...
ni_bulk = bulk("Ni", "fcc", a=3.524, cubic=True)
supercell = ni_bulk.repeat(2)        # 2x2x2 → 32 atoms
...
print(f"NN distance    : {nn_dist:.4f} Å  (expected ~2.492 Å for FCC Ni)")
...
neb.plot("ni_vacancy_emt_profile.png",
         title="Ni vacancy migration in FCC Ni [EMT, CI-NEB]")
neb.save_csv("ni_vacancy_emt_profile.csv")
neb.save_trajectory("ni_vacancy_emt.traj")
